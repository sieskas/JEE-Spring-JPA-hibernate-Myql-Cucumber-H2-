pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
                checkout([$class: 'GitSCM', branches: [[name: '*/master']], extensions: [[$class: 'LocalBranch', localBranch: 'master']], userRemoteConfigs: [[credentialsId: 'gitea', url: 'http://192.168.250.88:3000/storetraffic-backend/tmas.git']]])
            }
        }
                ...
                script {
                    ...
                    if (rebaseOutput.contains("error: could not apply")) {
                        ...
                        if (!branchExists) {
                            ...
                            bat 'curl -X POST "http://devhost:3000/api/v1/repos/SMS-Storetraffic/demo/pulls" -H "accept: application/json" -H "authorization: token b32cc126ab01300c0cf36f9134223b62c6d1cb9b" -H "Content-Type: application/json" -d "{\"assignee\": \"root\",\"assignees\": [],\"base\": \"' + newBranchName + '\",\"body\": \"\",\"due_date\": null,\"head\": \"dev\",\\"labels\": [],\"milestone\": null,\"title\": \"' + newBranchName '\"}"\n'
                        }
                    } else {
